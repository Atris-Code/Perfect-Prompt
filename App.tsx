...
export const App: React.FC = () => {
  ...
  const [isGovernanceCrisisActive, setIsGovernanceCrisisActive] = useState(true);
  ...
  const [stoStateForCreator] = useState<STOState>({
    assetId: 'AGS26-001',
    investors: [],
    fundsRaised: 0,
    target: 100000,
...
Descripción Metafórica: ${element.descripcion_metaforica || 'N/A'}
  ...
  const renderView = () => {
    switch (view) {
      ...
      case 'pro': return <ProStudio formData={{} as ProFormData} handleChange={()=>{}} handleSubmit={()=>{}} isLoading={false} error="" generatedPrompt="" onReset={()=>{}} onMetadataExtract={()=>{}} onUseInCreator={()=>{}} />;
      ...
      case 'process-optimizer': return <ProcessOptimizer onSaveTask={handleSaveTask} challengePackage={challengePackage} />;
      case 'property-visualizer': return <PropertyVisualizer setView={setView} setTasks={setTasks} setInitialLabMaterialIds={() => {}} />;
      case 'energy-balance': return <EnergyBalanceSimulator onNavigateWithContext={() => {}} generatedPrompts={{}} />;
      ...
      case 'phoenix': return <Phoenix phoenixState={phoenixState} setPhoenixState={setPhoenixState} argusModel={{datasetSize:15000, precision:98.5, falsePositiveRate:1.2}} onNavigateToUtilities={()=>{}} onSaveTask={handleSaveTask}/>;
      case 'vulcano': return <Vulcano vulcanoState={vulcanoState} setVulcanoState={setVulcanoState} setView={setView} onNavigateToUtilities={()=>{}} onSaveTask={handleSaveTask} />;
      case 'bioeconomy-lab': return isGovernanceCrisisActive ? <NexoControlPanel /> : <IAProStudio issuanceState={{contractStatus: 'UNDEPLOYED', tokenStatus: 'UNMINTED', contractAddress: null, events:[]}} handleDeployContract={()=>{}} handleMintTokens={()=>{}} gaiaLabState={gaiaLabState} onTakeSample={handleTakeSample} setView={setView} onSaveTask={handleSaveTask} />;
      case 'chronos': return <Chronos chronosState={{phase:1, assetOrigin:{assetType:'', estimatedVolume:0, marketValue:0, liquidationDate:'', status:'PENDING'}, tokenStructure:{rights:'',tokenName:'',nominalValue:0,totalSupply:0}, liquidation:{status:'PENDING', salePrice:null, profitPercentage:null}, secondaryAssets:[]}} issuanceState={{contractStatus:'UNDEPLOYED',tokenStatus:'UNMINTED',contractAddress:null,events:[]}} stoState={{assetId:null, investors:[], fundsRaised:0, target:100000, status:'PREPARING'}} setView={setView} handleVerifyAsset={()=>{}} handleLiquidation={()=>{}} />;
      case 'agriDeFi': return <AgriDeFi stoState={{assetId:null, investors:[], fundsRaised:0, target:100000, status:'PREPARING'}} onStartSTO={()=>{}} onSaveTask={handleSaveTask} />;
      case 'gaia-lab': return <GaiaLab gaiaLabState={{isNewBatchReady:false, sampleId:null, analysisResults:null, quality:null}} onTakeSample={()=>{}} />;
      case 'innovation-forge': return <InnovationForge coPresets={[]} reactors={[]} addEvent={()=>{}} />;
      ...
      case 'fleet-simulator': return <FleetSimulator onSaveTask={handleSaveTask} setView={setView} onCreateReport={()=>{}} onOpenUtilityWidget={()=>{}} onSimulationComplete={()=>{}} onNavigateToArchitecturalSynth={() => {}} />;
      case 'catalyst-lab': return <CatalystLab />;
      ...
      case 'energy-explorer': return <EnergyExplorer onSaveTask={handleSaveTask} />;
      ...
      case 'architectural-synthesis-dashboard': return <ArchitecturalSynthesisDashboard architecturalPreset={null} onPresetConsumed={() => {}} />;
  ...
  return (
    <div className="flex min-h-screen bg-gray-100">
      <aside className="w-64 bg-white flex-shrink-0 p-4 border-r border-gray-200">
