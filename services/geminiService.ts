...
export async function editImageWithPep(image: { data: string; mimeType: string; }, styleReference: string, action: string, technicalOutput: string, advancedControls: any): Promise<{ imageData?: string; text?: string }> {
    const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
    const prompt = `Edit the provided image. Action: ${action}. Style Reference: ${styleReference}. Technical Output: ${technicalOutput}. Advanced Controls: ${JSON.stringify(advancedControls)}`;
    ...
    if (resultPart?.inlineData) {
        return { imageData: resultPart.inlineData.data };
    }
...
export async function generateTextualCoherenceFeedback(formData: FormData): Promise<TextualNarrativeCoherence> {
    const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
    const prompt = `Analyze the textual coherence of this prompt idea. Data: ${JSON.stringify(formData)}. Evaluate stylistic cohesion, narrative architecture, and audience translation. Provide a score from -9 to +9 for each, and a brief analysis.`;
...
export async function optimizeProcess(material: PyrolysisMaterial, goal: string): Promise<OptimizationResult> {
    const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
    const prompt = `Optimize pyrolysis for ${material.nombre} with the goal: "${goal}". Return an OptimizationResult JSON object.`;
    const response = await ai.models.generateContent({
        model: 'gemini-2.5-pro',
        contents: prompt,
        config: { responseMimeType: "application/json" }
