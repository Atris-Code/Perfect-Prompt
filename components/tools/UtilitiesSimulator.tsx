...
`;
    ...
    return (
        <div className="bg-slate-900 text-white p-8 rounded-lg shadow-2xl h-full flex flex-col font-sans">
            <header className="text-center mb-8">
                <h2 className="text-3xl font-bold">{t('utilities.title')}</h2>
                <p className="mt-2 text-md text-slate-400">{t('utilities.subtitle')}</p>
            </header>
                ...
                <nav className="-mb-px flex space-x-4 overflow-x-auto" aria-label="Tabs">
                    {tabs.map(tab => (
                        <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                            className={`whitespace-nowrap py-4 px-2 border-b-2 font-medium text-sm ${activeTab === tab.id ? 'border-blue-500 text-blue-400' : 'border-transparent text-slate-400 hover:text-slate-200 hover:border-slate-500'}`}>
                            {tab.label}
                        </button>
                    ))}
                </nav>
