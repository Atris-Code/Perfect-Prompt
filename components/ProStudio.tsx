    ...
    const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            if (typeof window.exifr === 'undefined') {
              throw new Error("La librería de metadatos (exifr) no se pudo cargar. Comprueba tu conexión a internet y refresca la página.");
            }
            ...
            if (!data || Object.keys(data).length === 0) {
                throw new Error('No se encontraron metadatos EXIF en este archivo.');
            }
            setMetadata(data);
            ...
            if (errorMessage.toLowerCase().includes('unknown file format') || errorMessage.toLowerCase().includes('not a valid file')) {
                setMetadataError('Formato de archivo no reconocido o no compatible. La librería soporta la mayoría de formatos RAW y JPG/TIFF, pero este archivo específico no pudo ser procesado.');
            } else {
                setMetadataError(`No se pudo leer el archivo. Puede que esté corrupto o no sea un archivo de imagen. Detalle: ${errorMessage}`);
            }
            onMetadataExtract(null);
        } finally {
            setIsParsing(false);
                        ...
                        {generatedPrompt && (
                             <div className="mt-4 pt-4 border-t border-gray-600 space-y-3">
                                <div className="flex gap-3">
                                    <button onClick={() => onUseInCreator(generatedPrompt, ContentType.Texto)} className="flex-1 flex items-center justify-center bg-indigo-600 text-white font-bold py-2 px-3 rounded-lg hover:bg-indigo-700 transition-colors text-xs">Usar para Texto</button>
                                    <button onClick={() => onUseInCreator(generatedPrompt, ContentType.Imagen)} className="flex-1 flex items-center justify-center bg-indigo-600 text-white font-bold py-2 px-3 rounded-lg hover:bg-indigo-700 transition-colors text-xs">Usar para Imagen</button>
                                    <button onClick={() => onUseInCreator(generatedPrompt, ContentType.Video)} className="flex-1 flex items-center justify-center bg-indigo-600 text-white font-bold py-2 px-3 rounded-lg hover:bg-indigo-700 transition-colors text-xs">Usar para Video</button>
                                </div>
                                <div>
                                    <button
